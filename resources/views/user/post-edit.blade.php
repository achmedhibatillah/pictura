<div class="row m-0 p-0">
    <div class="col-md-7 m-0 p-0">
        <div class="card m-0 p-3">
            <p class="m-0">Edit post <i class="text-secondary fsz-12">(automatic change)</i></p>
            <div class="my-3">
            ...
            @if($slides->isNotEmpty())
                <hr>
                <form action="{{ url('edit-post/update') }}" method="post">
                    @csrf
                    <input type="hidden" name="post_id" value="{{ $post->post_id }}">
                    <button type="submit" class="td-hover fw-bold text-primary btn btn-transparent p-0">Return back</button>
                </form>
            @endif
        </div>
    </div>
